...
public class ServiceTypeExcelHelper {
    ...
    public static List<ServiceType> excelToServiceType(InputStream is, String sheetNumber) {
        try {
            ...
            while (rows.hasNext()) {
                ...
                while (cellsInRow.hasNext()) {
                    ...
                    switch (cellIdx) {
                        case 1:
                            serviceType.setLabel(currentCell.getStringCellValue());
                            break;
                        default:
                            break;
                    }
    ...
    public static List<ServiceType> csvToServiceType(InputStream is) {
        try {
            ...
            for (CSVRecord csvRecord : csvRecords) {
                ServiceType serviceType = new ServiceType();
                serviceType.setLabel(csvRecord.get("label"));
                listServiceType.add(serviceType);
            }
            return listServiceType;
        } catch (IOException e) {
    ...
    public static ByteArrayInputStream ServiceTypeToExcel(List<ServiceType> serviceTypes) {
        try {
            Workbook workbook = new XSSFWorkbook();
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            Sheet sheet = workbook.createSheet("ServiceTypes");
            ...
            for (ServiceType serviceType : serviceTypes) {
                ...
                row.createCell(0).setCellValue((Long)serviceType.getId());
                sheet.autoSizeColumn(0);
                row.createCell(1).setCellValue(serviceType.getLabel());
    ...
    public static ByteArrayInputStream ServiceTypeToCsv(List<ServiceType> serviceTypes) {
        try {
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            ...
            for (ServiceType serviceType : serviceTypes) {
                List<String> data = Arrays.asList(
                        String.valueOf(serviceType.getId()),
